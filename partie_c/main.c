...
void add_peer(struct sockaddr_in addr)
{
    ...
    {
        if (lobby[i].active && lobby[i].addr.sin_addr.s_addr == addr.sin_addr.s_addr)
            return;
        if (!lobby[i].active)
        {
            lobby[i].addr = addr;
...
{
// ---> L'INITIALISATION WINDOWS OUBLIÉE EST ICI <---
#ifdef _WIN32
    WSADATA wsaData;
    ...
    char buffer[sizeof(Message) + 10]; // Un peu de marge pour éviter les crashs
    ...
    while (1)
    {
        ...
        if (FD_ISSET(sock_ia, &reads))
        {
            ...
            if (len == sizeof(Message))
            {
                ...
                printf("[IA LOCALE] Action: %d recue pour %s (Taille OK)\n", m.action, m.target_id);
                for (int i = 0; i < MAX_PEERS; i++)
                {
                    if (lobby[i].active)
            ...
            {
                // ---> SÉCURITÉ : Affiche si les tailles Python/C ne matchent pas <---
                printf("[IA LOCALE] ❌ Erreur : Paquet recu de %d octets (attendu : %zu octets)\n", len, sizeof(Message));
            }
        }
        ...
        if (FD_ISSET(sock_res, &reads))
        {
            ...
            if (len == sizeof(Message))
            {
                ...
                else
                {
                    ...
                    addr_python.sin_port = htons(5001); // Le vrai port d'écoute du Python !
